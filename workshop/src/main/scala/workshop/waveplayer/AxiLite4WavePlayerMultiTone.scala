...
class AxiLite4WavePlayerMultiTone(
  ...
  val io = new Bundle {
    val axiLite = slave(
      AxiLite4(
        addressWidth = 2 * sampleCountLog2 * log2Up(toneCount),
        dataWidth = 32
      )
    )
    val wave = out(Sample)
  ...
  val tones = for (i <- 0 until toneCount) yield new Area {
    ...
    val factoryWithOffset =
      new BusSlaveFactoryAddressWrapper(factory, i * 2 * sampleCountLog2)
  ...
  io.wave := tones
    .map(tone =>
      tone.wavePlayer.phase.run ? tone.wavePlayer.filter.value | U(0)
    )
    .reduce(_ + _)
}
