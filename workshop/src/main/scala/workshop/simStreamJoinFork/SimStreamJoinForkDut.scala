...
class SimStreamJoinForkDut extends Component {
  ...
  io.rspMul << cmdMul
    .translateWith(cmdMul.a * cmdMul.b + (if (errorId == 2) 4 else 0))
    .stage()
    .stage()
  io.rspXor << cmdXor
    .translateWith(cmdXor.a ^ cmdXor.b ^ (if (errorId == 1) 4 else 0))
    .s2mPipe()

  if (errorId == 3)
    cmdABuffer.ready setWhen (cmdABuffer.payload(2 downto 0) === 2)
}
