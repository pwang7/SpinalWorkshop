...
class ApbPwmTester extends AnyFunSuite {
  ...
  test("testbench") {
    compiled.doSim { dut =>
      ...
      val io =
        dut.io.flatten.map(e => e.getName().replace("io_", "") -> e).toMap
      ...
      def apbReadAssert(
          ...
          mask: BigInt,
          message: String
      ): Unit = assert((apbRead(address) & mask) == data, message)
      ...
      apbReadAssert(
        ...
        0x00000001,
        0x00000001,
        "APB pwm enable read doesn't work properly."
      );
      apbReadAssert(
        0x04,
        0x00000040,
        0x000000ff,
        "APB duty cycle read doesn't work properly."
      );
