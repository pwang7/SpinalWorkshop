...
class StreamUnitTester extends AnyFunSuite {
  ...
  test("testbench") {
    compiled.doSimUntilVoid(seed = 42) { dut =>
      ...
      StreamDriver(dut.io.cmdA, dut.clockDomain) { p =>
        p.randomize(); ramLoaded
      }
      StreamDriver(dut.io.cmdB, dut.clockDomain) { p =>
        p.randomize(); ramLoaded
      }
      ...
      StreamMonitor(dut.io.cmdA, dut.clockDomain) { p =>
        aHistory.enqueue(p.toInt)
      };
      StreamMonitor(dut.io.cmdB, dut.clockDomain) { p =>
        bHistory.enqueue(p.toLong)
      }
      StreamMonitor(dut.io.rsp, dut.clockDomain) { p =>
        assert(
          aHistory.nonEmpty,
          "Got a rsp transaction without cmdA transaction"
        )
        assert(
          bHistory.nonEmpty,
          "Got a rsp transaction without cmdB transaction"
        )
        ...
        assert(
          p.toLong == ref,
          f"rsp had the wrong value from cmdA=$a cmdB=$b%x mem(cmdB)=${ramContent(a)}%x"
        )
      ...
      for (i <- 0 to 255) {
        assert(
          dut.mem.getBigInt(i) == ramContent(i),
          s"dut.mem($i) wasn't written properly"
        )
      }
      ramLoaded = true
    }
