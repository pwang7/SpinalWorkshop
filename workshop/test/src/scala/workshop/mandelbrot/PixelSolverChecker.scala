...
object PixelSolverChecker {
  ...
  def apply(
      cmd: Stream[PixelTask],
      rsp: Stream[PixelResult],
      cd: ClockDomain
  ): Unit = {
    SimTimeout(2000000)
    ...
    fork {
      ...
      val ref = "workshop/test/src/scala/workshop/mandelbrot/mandelbrot.ref"
      var error = false
      for ((refLine, i) <- Source.fromFile(ref).getLines.zipWithIndex) {
        val dutLine = dutLines(i)
        if (dutLine != refLine) {
          // println(s"dutLine=$dutLine\nrefLine=$refLine")
          error = true
        }
      }
      ...
      //GUI
      val displayEnv = sys.env.get("DISPLAY")
      val frame = displayEnv.map { _ =>
        new JFrame {
      ...
      while (true) {
        if (frame == None || frame.get.closed) {
